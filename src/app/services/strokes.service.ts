...
export class StrokesService {
  ...
  //       columnDef: 'name1',
  //       header: "Name",
  //       cell: (element: Results) => `${element.name}`,
  //     },
  //     {
  //       columnDef: 'scores[0]',
  //       header: (data: Results) => `${data.scores[0]}`,
  //       cell: (element: Results) => `${element.scores[0]}`,
  //     },
  ...
  teamMatch(foursome, j) {  //Create a row that shows the fourball match status.  Assume auto presses when two down and 18-hole press when closed out
  ...
  matchPress(A, k) {  //Calculate the 18-hole press result A is the array of all scores inthe match k is the index of the player
                      //  Assumes players are arranged in foursomes
    let matchScore: number = 0;
    let matchScorePress: number = 0;
    let i = 0; //Hole counter
    while (Math.abs(matchScore) <= 17 - i) {  //Track match status after each hole
    ...
    for (let i = 17 - p + 2; i < 18; i++) {
      if (A[k].betterBallNet[i] > A[k + 2].betterBallNet[i]) {  //If match is closed out, start a press
        matchScorePress--;
      }
      if (A[k].betterBallNet[i] < A[k + 2].betterBallNet[i]) {
        matchScorePress++;
